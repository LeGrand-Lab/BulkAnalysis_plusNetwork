...
def filldicoSenders(dex, daydf, allcelltypes, day): 
    ...
    ex = ex[['Ligand symbol','Sending cluster','Ligand derived specificity of average expression value']]
    ...
    for i,row in ex.iterrows():
         ...
         dex[sym][row['Sending cluster']][dki[day]] = row['Ligand derived specificity of average expression value']
    return dex
                
def seevariances_setcutoff(dex, QUANT4var, QUANT4exp ):   
    """
    helps to seek for best cutoffs : 
        - a variance cutoff when more than 2 real specificities values (not -1)
        - an expression cutoff if only 1 real specificity
    """
    ...
    plt.title("variances of Ligands specs across days")     
    plt.xlabel("variance")
    plt.ylabel("n")
    plt.show()
...
def pickovercutoff(dex, varcutoff, expcutoff):
    ...
    yields a list of selected (over cutoffs) tuples : 
        (a_symbol, a_celltype, the_varianceORSpecif, a_criterion_cutoff)
    """
    selection = []
    for k in dex.keys():
        for ct in dex[k]:
            ...
            if len(realvalues) > 1 :
                ...
                if realvalues[0] > expcutoff:
                    selection.append((k, ct, realvalues[0], "SPECBASED"))
    return selection
