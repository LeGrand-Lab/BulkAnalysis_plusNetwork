     ...
     step 1 : 
         - create sender NESTED dict using Natmi output dataframes all days :  
           external keys are the gene symbols
             take 'Ligand derived specificity of average expression value'   
                  keys celltypes associated to list specs values (4values)
        - select, across sequential days,  MOST VARIABLE ligands (variance cutoff)
              (Note: if value for only one day, filter by specs cutoff)
     step 2 :
...
import copy
from boxFunClassVersion2 import *
...
varcutoff, expcutoff = seevariances_setcutoff(ds, 0.85, 0.75) # QUANTILEs best tuned
print(varcutoff)
mysele = pickovercutoff(ds, varcutoff, expcutoff)
texto = "symbol\tcellty\tvarianceORspecs\tcriterionselection\n"
for i in mysele:
    texto += '\t'.join([str(x) for x in i]) 
    texto += '\n'               

with open("selectedLigandsVersion2.txt", "w") as f:
    f.write(texto)
    f.close()
