...
dim(all_g_df) # 1609 because no filter in padj
all_g_df <- all_g_df %>% filter(repadj <= 0.5) # keep no signif also for GSEA

# this selection for GO 
g_df <- all_g_df %>% filter(abs(log2FoldChange)>=1.2 & repadj <= signifcutoff) 
...
gg_go <- ggplot(refilter, 
  ...
  labs(subtitle = paste("GO/Pathways enrichment"),
       caption = paste("Young vs Old,",ct,
            "most significant (green), min FDR=",round(min(refilter$FDR),5)))


# ################### do for all genes in TOP !! pathways:
## transform into two grids :  the new with all diffDE to a new grid
GG = gp_mod %>% 
...
gg_gen <- ggplot(mtmelt, aes(x=day, y=Zscore, group=age , color=age))  +
  ...
  labs(subtitle=paste(ct,"Old vs Young representative genes"),
       caption=paste("differentialDE filter padj<=",signifcutoff))
