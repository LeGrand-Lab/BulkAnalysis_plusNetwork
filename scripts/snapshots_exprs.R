...
# print stuff:
pdf(paste0("plotsDE/volcano_snapshot.pdf"), width=14, height=10)
...
# 
# ggplot(sortie, aes(x=log2FoldChange, y = -log10(padj+1e-20))) +
#   geom_point(aes(color=DEsignificant),size=.2) +
#   scale_color_manual(values=c(col_vir[7], col_vir[3],"gray")) +
#   facet_grid(vars(day),vars(type),
#              labeller=labeller(type=nbDE.vec)) + 
#   theme_calc() +
...
#     max.overlaps=15
#   ) +
#   geom_text_repel(data=subset(sortie, padj < 0.005 & 
#                                 abs(log2FoldChange) > 1.2 &
#                                 type %in% c("M1", "M2", "Neutro")) ,
#                   aes(label=symbol, color=DEsignificant),
#                   size=2, segment.size = .1,
#                   force =2, 
#                   force_pull = 2, max.overlaps = 15
#   ) +  geom_text_repel(data=subset(sortie, padj < 0.005 & 
#                                      abs(log2FoldChange) > 1.2 &
#                                      type =="ECs") ,
#                        aes(label=symbol, color=DEsignificant),
