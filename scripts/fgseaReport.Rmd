...
# Report
#Existing
reportDEGanalysis<-paste0(dir,"reports/GeneDEsigniOnMoreOneTimePoint.html")
reportDEGtable<-paste0(dir,"reports/tableDEGsignificantUpDown.html")
...
for (t in Typecellv){
  link=paste0("[", GSEAsigni %>% filter(type==t) %>% select(pathway) %>% unique() %>% unlist() %>% length(),"](",dir,reportsuppHierarchie,"/",t,"_Hierarchical_Figsupp.html)")
  NumberPathwaysUniqueOnOneTypeCell<-c(NumberPathwaysUniqueOnOneTypeCell,link)
  for (d in daysv[[t]]){
    link=paste0("[",dim(GSEAsigni %>% filter(day==d & type==t))[1],"](",dir,reportsuppHierarchie,"/",d,'_',t,"_Hierarchical_Figsupp.html)")
    if ( dim(GSEAsigni %>% filter(day==d & type==t))[1] == 0){
      NumberPathwaysSigni[d, t] <-0
      }
    else{NumberPathwaysSigni[d, t] <- link}
    if (t == Typecellv[1]){
      link=paste0("[",GSEAsigni %>% filter(day==d) %>% select(pathway) %>% unique() %>% unlist() %>% length(),"](",dir,reportsuppHierarchie,"/",d,'_',"_Hierarchical_Figsupp.html)")
