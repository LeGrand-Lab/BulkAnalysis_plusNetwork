...
library(DT)
library(ggrepel)
library(ggnewscale)# for labels to point
 library(circlize)
...
Inside these differents genes, there are (cf table below) number of genes  in the enrichment.
...
typebytime<-ggplot( params$tablePlot, aes(x=day,y=as.numeric(NES),fill=type,color=type,group=paste0(pathway,'_',type))) +
    ...
    scale_color_manual(values=params$tablePlot %>% arrange(type) %>% select(color) %>% unique() %>% unlist() %>% as.character())+
...
## Heatmap of the top 40 genes DE between old versus young

--------

```{r Heatmap, echo=FALSE,warning=FALSE,message=FALSE, fig.show='asis'}

reactomeGSEAheatmap<-draw (params$Heatmplot, column_title = params$title,
                           heatmap_legend_side = "bottom", padding = unit(c(2, 2, 2, 60), "mm"))

```

## Expression of genes DE in this pathway through days and type cells for young versus old 
...
PlotGenelogFacrossDay <- ggplot(params$tablePlottest, aes(x=DayAge, y=log2(meanCountNormalized+1),  color=log2F, group=GeneDayType)) +
      ...
      new_scale("color") +
      geom_text_repel(data= params$subset,
...
tablePlotfilter<-params$tablePlottest %>% select(symbol,DayType,day,type,age,padj,log2F,meanCountNormalized) %>% arrange(symbol)
